...
start() {
    ...
    sudo -u {{User}} QUILT_HOME=$QUILT_HOME $GOPATH/bin/{{Component}} -c $COMPONENT_HOME/resource/config/config.json,$COMPONENT_HOME/resource/config/parameters.json  >> "{{StdOutLog}}" 2>> "{{StdErrLog}}" &
    echo $! > $PID_FILE
    success
    echo
}
